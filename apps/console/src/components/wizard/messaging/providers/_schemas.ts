...
const mailgunSchema = y.object().shape({
  ...
  fromName: y.string().required("From name is required"),
...
const sendgridSchema = y.object().shape({
  ...
  sendgridApiKey: y.string().required("SendGrid API key is required"),
  fromName: y.string().required("From name is required"),
  fromEmail: y.string().email("Valid email address required").required("From email is required"),
});
...
const smtpSchema = y.object().shape({
  ...
  smtpPort: y.number().min(1).max(65535).required("Valid SMTP port is required"),
  smtpUsername: y.string().required("SMTP username is required"),
  smtpPassword: y.string().required("SMTP password is required"),
  smtpSecure: y.boolean().optional(),
  fromName: y.string().required("From name is required"),
  fromEmail: y.string().email("Valid email address required").required("From email is required"),
});
...
const msg91Schema = y.object().shape({
  ...
  msg91ApiKey: y.string().required("MSG91 API key is required"),
  msg91SenderId: y.string().required("MSG91 Sender ID is required"),
  msg91Route: y.string().required("MSG91 Route is required"),
});
...
const fcmSchema = y.object().shape({
  ...
  fcmServerKey: y.string().required("FCM Server Key is required"),
  fcmSenderId: y.string().required("FCM Sender ID is required"),
  fcmProjectId: y.string().required("FCM Project ID is required"),
});
