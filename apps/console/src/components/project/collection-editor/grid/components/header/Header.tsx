...
const DefaultHeader = () => {
  ...
  const onAddRow =
    snap.editable && (snap.collection.attributes ?? []).length > 0
      ? tableEditorSnap.onAddRow
      : undefined;
  ...
  const onApplyFilters = useCallback(
    ...
    [projectRef, snap.collection.name, snap.collection.$schema, setParams],
  );
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent side="bottom" align="start">
                  {[
                    ...
                    //           <p className="neutral-on-background-medium">Insert new rows from a CSV</p>
                    //         </div>
                    //       </DropdownMenuItem>,
                    //     ]
                    //   : []),
...
const RowHeader = ({ sorts, filters }: RowHeaderProps) => {
  ...
  //     ? toast(
  //       <>
  //         <SonnerProgress progress={0} message={`Exporting all rows from ${snap.collection.name}`} />,
  //       </>,
  //       {
  //         closeButton: false,
  //         duration: Infinity,
  ...
  //     )
  //     : toast.loading(
  //       `Exporting ${snap.selectedRows.size} row${snap.selectedRows.size > 1 ? "s" : ""} from ${snap.collection.name}`,
  //     );
  ...
  //           <SonnerProgress
  //             progress={progress}
  //             message={`Exporting all rows from ${snap.collection.name}`}
  //           />,
  //           {
  //             id: toastId,
  //             closeButton: false,
      ...
      {/* <DropdownMenu>
        ...
        <DropdownMenuContent className="w-40">
          <DropdownMenuItem onClick={onRowsExportCSV}>
            <span className="neutral-on-background-medium">Export to CSV</span>
          </DropdownMenuItem>
