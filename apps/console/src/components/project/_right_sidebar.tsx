...
export const RightSidebar = () => {
  ...
  return (
    ...
    >
      <div className={cn("gap-6 flex flex-col visible opacity-100 ease-in transition-opacity duration-300", { "invisible opacity-30": !open })}>
        {open && (
          <>
      ...
      <button className={cn("neutral-background-weak hover:!bg-[var(--surface-background)] cursor-pointer flex items-center justify-center rounded-full size-6 p-1 transition-all duration-300 mt-auto", {
        "w-12 h-6 shadow-md z-2 rounded-l-full rounded-r-none justify-start pl-2": !open,
        "border neutral-border-medium": open
      })}
        onClick={() => setOpen(!open)}
      >
        <Icon name={"chevronRight"} size="s" className={cn("transition-all duration-300", { "rotate-180": !open })} />
      </button>
    </Column>
  );
};
