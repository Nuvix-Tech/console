...
export const generateCascadeActionDescription = (
  ...
  switch (cascadeAction) {
    case FOREIGN_KEY_CASCADE_ACTION.NO_ACTION:
      return (
        <>
          <span className="neutral-on-background-medium">{actionName}</span>: {actionVerb} a record
          from <Code className="text-xs neutral-on-background-medium">{reference}</Code> will{" "}
    ...
    case FOREIGN_KEY_CASCADE_ACTION.RESTRICT:
      return (
        <>
          <span className="neutral-on-background-medium">{actionName}</span>
          <Tooltip>
            <TooltipTrigger className="translate-y-[3px] mx-1">
              <HelpCircle className="neutral-on-background-medium" size={16} strokeWidth={1.5} />
            </TooltipTrigger>
          ...
          <Code className="text-xs neutral-on-background-medium">{reference}</Code> will{" "}
    ...
    case FOREIGN_KEY_CASCADE_ACTION.SET_DEFAULT:
      return (
        <>
          <span className="neutral-on-background-medium">{actionName}</span>: {actionVerb} a record
          from <Code className="text-xs neutral-on-background-medium">{reference}</Code> will set
          the value of any existing records in this table referencing it to their{" "}
    ...
    case FOREIGN_KEY_CASCADE_ACTION.SET_NULL:
      return (
        <>
          <span className="neutral-on-background-medium">{actionName}</span>: {actionVerb} a record
          from <Code className="text-xs neutral-on-background-medium">{reference}</Code> will set
          the value of any existing records in this table referencing it{" "}
