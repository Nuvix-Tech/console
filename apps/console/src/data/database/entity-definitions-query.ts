...
export async function getEntityDefinitions(
  { projectRef, sdk, schemas, limit }: EntityDefinitionsVariables,
  signal?: AbortSignal,
) {
...
export const useEntityDefinitionsQuery = <TData = EntityDefinitionsData>(
  { projectRef, sdk, schemas, limit }: EntityDefinitionsVariables,
  {
    enabled = true,
    ...options
  }: QueryOptions<EntityDefinitionsData, EntityDefinitionsError, TData> = {},
) =>
  useQuery({
    queryKey: databaseKeys.entityDefinitions(projectRef, schemas),
    queryFn: ({ signal }) => getEntityDefinitions({ projectRef, sdk, schemas, limit }, signal),
