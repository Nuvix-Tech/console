...
export async function getTableConstraints(
  { projectRef, sdk, id }: TableConstraintsVariables,
  signal?: AbortSignal,
) {
  const sql = getTableConstraintsSql({ id });
  const { result } = await executeSql(
    { projectRef, sdk, sql, queryKey: ["table-constraints", id] },
    signal,
  );
...
export const useTableConstraintsQuery = <TData = TableConstraintsData>(
  { projectRef, sdk, id }: TableConstraintsVariables,
  {
    enabled = true,
    ...options
  }: QueryOptions<TableConstraintsData, TableConstraintsError, TData> = {},
) =>
  useQuery({
    queryKey: databaseKeys.tableConstraints(projectRef, id),
    queryFn: ({ signal }) => getTableConstraints({ projectRef, sdk, id }, signal),
