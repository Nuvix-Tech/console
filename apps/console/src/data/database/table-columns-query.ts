...
export async function getTableColumns(
  { projectRef, sdk, table, schema }: TableColumnsVariables,
  signal?: AbortSignal,
) {
  const sql = getTableColumnsSql({ table, schema });

  const { result } = await executeSql(
    { projectRef, sdk, sql, queryKey: ["table-columns", schema, table] },
    signal,
  );
...
export const useTableColumnsQuery = <TData = TableColumnsData>(
  { projectRef, sdk, schema, table }: TableColumnsVariables,
  { enabled = true, ...options }: QueryOptions<TableColumnsData, TableColumnsError, TData> = {},
) =>
  useQuery({
    queryKey: databaseKeys.tableColumns(projectRef, schema, table),
    queryFn: ({ signal }) => getTableColumns({ projectRef, sdk, schema, table }, signal),
