...
import { AppRouterInstance } from '@bprogress/next'
...
function createTabsState(projectRef: string) {
  ...
  const store = proxy({
    ...
    handleTabNavigation: (id: string, router: AppRouterInstance) => {
      const { id: projectId } = useParams(); // TODO: ---------------
      ...
      switch (tab.type) {
        case 'sql':
          const schema = (query.get('schema') as string) || 'public'
          router.push(`/project/${projectId}/sql/${tab.metadata?.sqlId}?schema=${schema}`)
          break
        case 'r':
        case 'v':
        case 'm':
        ...
        case 'p':
          router.push(
            `/project/${projectId}/editor/${tab.metadata?.tableId}?schema=${tab.metadata?.schema}`
          )
          break
      }
    },
    ...
    handleTabCloseAll: ({
      ...
      onClearDashboardHistory()
      router.push(`/project/${projectId}/${editor === 'table' ? 'editor' : 'sql'}`)
    },
...
export const TabsStateContextProvider = ({ children }: PropsWithChildren) => {
  ...
  const state = useConstant(() => createTabsState(project?.$id ?? ''))
