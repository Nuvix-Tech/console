...
Store user settings that persist across sessions. Nuvix makes it simple to save themes, languages, notifications, and any custom preferences for both individual users and teams.
...
// Update team preferences
const teamPrefs = await teams.updatePrefs('<TEAM_ID>', {
    theme: 'corporate',
...
// Get team preferences
const teamPreferences = await teams.getPrefs('<TEAM_ID>');
