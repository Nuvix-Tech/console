...
Initialize your Nuvix SDK's `Client`.
You'll use Avatars API to generate a QR code for the TOTP authenticator app.
...
Generate the recovery codes by calling `nx.account.createMfaRecoveryCodes()`.

<CodeGroup>
```javascript JavaScript/TypeScript
const response = await nx.account.createMfaRecoveryCodes();
...
<CodeGroup>
```javascript JavaScript/TypeScript
const response = await nx.account.updateEmail({
...
Then, initiate verification for the email by calling `nx.account.createEmailVerification()`.
...
<CodeGroup>
```javascript JavaScript/TypeScript
const res = await nx.account.createVerification({
...
Finally, verify the email by calling `nx.account.updateVerification()` with `userId` and `secret`.

<CodeGroup>
```javascript JavaScript/TypeScript
const response = await nx.account.updateVerification({
    userId: '<USER_ID>',
    secret: '<SECRET>'
});
```
...
<CodeGroup>
```javascript JavaScript/TypeScript
const response = await nx.account.updatePhone({
    phone: '+12065550100',
    password: 'password'
});
```
...
Then, initiate verification for the phone number by calling `nx.account.createPhoneVerification()`.

<CodeGroup>
```javascript JavaScript/TypeScript
const response = await nx.account.createPhoneVerification();
```
</CodeGroup>

After the user receives the verification code, they can verify their phone number by calling `nx.account.updatePhoneVerification()`.

<CodeGroup>
```javascript JavaScript/TypeScript
const response = await nx.account.updatePhoneVerification({
    userId: '<USER_ID>',
    secret: '<SECRET>'
});
```
...
First, add a TOTP authenticator to the user's account by calling `nx.account.addAuthenticator()`.

<CodeGroup>
```javascript JavaScript/TypeScript
const { secret, uri } = await nx.account.createMfaAuthenticator({
    type: 'totp'
});
```
</CodeGroup>
...
<CodeGroup>
```javascript JavaScript/TypeScript
const result = await nx.avatars.getQR({
    text: uri,
    size: 800,  // optional
    margin: 0,  // optional
...
verify the authenticator by calling `nx.account.verifyMfaAuthenticator()`.

<CodeGroup>
```javascript JavaScript/TypeScript
const promise = nx.account.updateMfaAuthenticator({
    type: 'totp',
    otp: '<OTP>'
});
```
...
You can enable MFA on your account by calling `nx.account.updateMFA()`.
...
Upon successful login in the first authentication step, check the status of the login by calling `nx.account.get()`.
...
```javascript JavaScript/TypeScript
try {
    const response = await nx.account.get();
...
You can check which factors are enabled for an account using `nx.account.listMfaFactors()`.
...
<CodeGroup>
```javascript JavaScript/TypeScript
const response = await nx.account.updateMfaChallenge({
...
You can confirm this by running `nx.account.get()`

## 9. Recovery

In case your user needs to recover their account, they can use the recovery codes generated in the first step with the
recovery code factor. Initialize the challenge by calling `nx.account.createMfaChallenge()` with the factor `recoverycode`.
...
Then complete the challenge by calling `nx.account.updateMfaChallenge()` with the challenge ID and the recovery code.
