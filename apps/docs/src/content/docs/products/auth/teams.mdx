...
const memberships = await teams.listMemberships('developers');
...
```javascript
await teams.deleteMembership('developers', 'membership123');
```
...
<Warning>
Privacy settings apply globally to all teams. Consider your application's privacy requirements when configuring these settings.
</Warning>
...
customerTeam = await teams.create(
    'customer-123',
    'Acme Corp',
    ['admin', 'user', 'viewer']
);
...
workspaceTeam = await teams.create(
    'workspace-alpha',
    'Alpha Workspace', 
...
storage.createFile('documents', 'doc123', file, [
