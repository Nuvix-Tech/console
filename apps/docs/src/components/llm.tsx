...
import { ChevronDown, ExternalLinkIcon, Copy } from "lucide-react";
...
export function ViewOptions(props: { markdownUrl: string; githubUrl?: string; markdown?: string }) {
  ...
  const actions: Array<{ title: string; description: string; href: string; icon: ReactElement }> =
    [];

  if (props.githubUrl) {
    actions.push({
      title: "Open in GitHub",
      description: "View the source page on GitHub",
  ...
  actions.push(
    {
      ...
      description: "Ask questions using ChatGPT",
      href: gpt,
      icon: (
    ...
    },
    {
      ...
      description: "Ask questions using Claude",
      href: claude,
      icon: (
  ...
  return (
    <ButtonGroup>
      ...
      <Popover>
        ...
        <PopoverContent className="flex flex-col overflow-auto gap-1 p-2">
          <button onClick={copyPage} className={cn(optionVariants(), "cursor-pointer")}>
            <Icon name={Copy} size="s" />
            <div className="flex flex-1 flex-col items-start">
              <span className="font-medium">Copy Page</span>
              <span className="text-fd-muted-foreground text-xs">Copy page in markdown</span>
            </div>
          </button>
          ...
          {actions.map((item) => (
            ...
            >
              ...
              <div className="flex flex-1 flex-col">
                ...
                <span className="text-fd-muted-foreground text-xs">{item.description}</span>
              </div>
              <Icon
                name="externalLink"
                size="m"
