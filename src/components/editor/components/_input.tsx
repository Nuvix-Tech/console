...
import { Input as Input_Shadcn_ } from "@/components/ui/input";
...
>(
  (
    ...
    const __styles = {
      base: "relative",
      with_icon: "pl-10",
      actions_container: "absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2",
    };
    ...
    return (
      <div className={cn("relative group", containerClassName)}>
        ...
        />
        {icon && <Icon name={icon} className={iconContainerClassName} />}
        {copy || actions ? (
          <div className={__styles.actions_container}>
            ...
            {copy && !(reveal && hidden) ? (
              <Button
                ...
                suffixIcon={<Copy size={16} className="text-foreground-muted" />}
            ...
            {reveal && hidden ? (
              <Button size="s" type="default" onClick={onReveal}>
                Reveal
              </Button>
            ) : null}
            {actions && actions}
